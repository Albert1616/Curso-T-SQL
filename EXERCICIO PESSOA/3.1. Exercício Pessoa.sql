...
--WHERE Nome LIKE 'E%' OR Nome LIKE '%[eéê]'
...
--WHERE CHARINDEX('José', Nome) > 0 OR CHARINDEX('Jose', Nome) > 0 -- MELHOR COM LIKE

-- 15

--SELECT MAX(LEN(Nome)) AS Maior FROM Pessoa - MAX, LEN
--WHERE Nome LIKE '%[José, josé, jose]'
...
--RIGHT(Nome,4) AS 'Último nome' FROM Pessoa
